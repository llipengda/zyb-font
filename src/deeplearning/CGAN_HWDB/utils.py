...
def read_img(path: str) -> Image.Image:
    try:
        ...
        return Image.open(f'data/CGAN_HWDB/SIMHEI.TTF/{path.split(os.sep)[-1]}').convert('1')
    ...
    for y in range(height):
        for x in range(width):
            ...
            if pixel == 0:
                ...
                if all(p is None or p >= 100 or y == 1 or y == 0 for p in [left_pixel, right_pixel, top_pixel, bottom_pixel]):
